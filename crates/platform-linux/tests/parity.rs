...
/// stdout writes to a real output file — proving the dup'd fd actually
/// carries the right bytes both ways, and that the caller's own file
/// handles remain open and independently readable afterward (the "caller
/// keeps their own copy" half of the contract).
#[cfg(target_os = "linux")]
#[test]
fn linux_stdio_file_redirect_reads_and_writes_through_real_files() {
    ...
    let mut input = root
        ...
        .expect("create input");
    input.write(b"redirected input").expect("write input");
...
#[cfg(target_os = "linux")]
#[test]
fn linux_stdio_file_from_wrong_backend_fails_cleanly() {
    use platform::process::{Command, Spawner, Stdio};
